...
_gui.switch.addEventListener("click", () => {
   ...
   //console.log(_data.gameOn);//mostra se está ligado ou desligado

   _gui.counter.classList.toggle("gui__counter--on"); //o display acende e apaga
...
_gui.strict.addEventListener("click", () => {
    if(!_data.gameOn) { //impede de ligar o strict quando o game esta desligado
        return;
    }
...
const blink = (text, callback) => {//mostrar o score.
    let counter = 0,
        on = true;
